...
def pred(file, target):
    ...
    df_out = df_out.drop(columns=['tip_y', 'size_y', 'sex_Female', 
                                ...
                                'time_Dinner', 'time_Lunch'], axis=1) # drop encoded columns
    ...
    # rename columns 
    df_out.rename(columns = {"tip_x": "tip", "size_x": "number of guests", "total_bill": "total bill"}, inplace = True)
    df_out_preds.rename(columns = {"tip_x": "tip", "size_x": "number of guests", "total_bill": "total bill"}, inplace = True)
    print(df_out) 
...
exp.info_consent.introduction += al.Text(path="introduction.txt") #TODO: fill page w/text 
    ...
    def on_first_show(self):
        if int(self.exp.values.get("participant_age")) <= 18:
            self += al.Text(path="debrief.txt", align="center")
             ...
             self += al.Text(path="Participant Information.txt") # TODO: ask about participant info 
...
class Instructions(al.Page): 
    ...
    def on_first_show(self):
        ...
        self += al.Html(html="""
                        ...
                        """, position="center")
        self += al.VerticalSpace("5mm")
        self += al.Text("How much do you think was paid as a tip for this bill? (Please enter a number between 0-10, to two decimal places e.g. 1.00)", name="mt", align="center")
        self += al.Hline()
...
class Experience(al.Page):
    ...
    def on_exp_access(self):
        ...
        #TODO: fix formatting of text ;w; 
        self += al.Text("""
                        Next, you will complete 10 practice estimates. This is just to give you experience before you complete your 10 actual estimates.<br> 
                        You will see the data table with information about each bill. You will make an estimate of the total bill.<br>
...
# part 2+bonus info 
exp.instructions_section += al.Page(name="Part2")
exp.instructions_section.Part2 += al.Text(path="task_instructions.txt")
exp.instructions_section.Part2 += al.TextArea(toplab="Please type the underlined portion of the text above into the box below to show that you have read the incentive/bonus information.", name="bonus_info")
...
class OPG1(al.Page):
    ...
    def on_first_show(self):
        ...
        self += al.Hline()
        self += al.NumberEntry(toplab="How much do you think was paid total for this bill?", min=0, max=10, name=f"prediction_{item+1:02}", align="center")
...
class OPG2(al.Page):
    ...
    def on_first_show(self):
        ...
        self += al.SingleChoiceButtons("None", "Little", "Some", "A Fair Amount", "A Lot", toplab="How confident are you in the accuracy of your second estimate?", name=f"b1_{item+1:02}")
